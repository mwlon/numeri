...
const { matMulVec, vecMulMat, outerProdInto, matMul, dot } = require('./matrixUtils');
...
module.exports = {
  ...
  symHessenberg(tensor, options) {
    ...
    for (let i = 0; i < n - 2; i++) {
      ...
      const offDiagEntry = hessenberg.get(i + 1, i) -
        2 * householder.get(0) *
          dot(householder, hessenberg.slice([i + 1, n], i));
  ...
  symEig(tensor, options) {
    ...
    const { vals, vecs } = module.exports.symTridiagEig(hessenberg, options);
