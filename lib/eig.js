...
module.exports = {
  symHessenberg(tensor, options) {
    ...
    for (let i = 0; i < n - 2; i++) {
      ...
      submat.assign(submat.minus(
        rowAdjustment.times(2)
      ));
      ...
      if (includeQ) {
        ...
        const qAdjustment = qSubmat
          .matMul(vCol)
          .matMul(vRow);
        qSubmat.assign(qSubmat.minus(qAdjustment.times(2)));
      }
    }
  ...
  tridiagonalEig(tensor, options) {
    ...
    for (let startI = 0; startI < n - 1; startI++) {
      let deviation;
      for (let iter = 0; iter < maxIter && (deviation === undefined || deviation > tolerance); iter++) {
        deviation = module.exports.tridiagonalQrIter(toDiagonalize, vecs, startI);
      }
    }
  ...
  tridiagonalQrIter(toDiagonalize, vecs, startI) {
    ...
    for (let i = startI; i < n; i++) {
      toDiagonalize.set([i, i], toDiagonalize.get(i, i) - shift);
    }
    ...
    for (let i = startI; i < n - 1; i++) {
      ...
      if (vecs) {
        const rotMat = createUtils.fromFlat(
          [cos, -sin, sin, cos],
          [2, 2]
        );
        const vecsSlice = vecs.slice(undefined, [i, i + 2]);
        vecsSlice.assign(vecsSlice.matMul(rotMat));
      }
