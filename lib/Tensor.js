...
class Tensor {
  ...
  setup() {
    ...
    const getIByJ = this.getIByJGetter();
  ...
  getIByJGetter() {
    ...
    if (this.isSimple()) {
      ...
      const { mods: jMods, strides: jStrides } = tensorUtils.getStridesAndMods(shape);
  ...
  setAll(val) {
    this.elementwiseUnaryOpInPlace(() => val);
  }
  ...
  elemwiseBinaryOpUnsafe(other, op) {
    const { getByJ: thisGetByJ } = this;
    const { getByJ: otherGetByJ } = other;
    const length = this.getLength();
    const newData = Array(length);
  ...
  matMul(other) {
    ...
    for (var loc0 = 0; loc0 < this.shape[0]; loc0++) {
      for (var k = 0; k < this.shape[1]; k++) {
        const thisElem = this.get(loc0, k);
        const rowOffset = loc0 * other.shape[1];
        for (var loc1 = 0; loc1 < other.shape[1]; loc1++) {
          data[rowOffset + loc1] += thisElem * other.get(k, loc1);
        }
      }
    }
