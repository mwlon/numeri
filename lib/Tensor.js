  ...
  slice(...specs) {
    ...
    for (var idx = 0; idx < this.dims; idx++) {
      ...
      if (spec === undefined) {
        ...
        let [ start = 0, end = this.shape[idx], step = 1 ] = spec;
        start = Math.max(start, 0);
        end = Math.min(end, this.shape[idx]);
  ...
  matMul(other) {
    ...
    for (var i = 0; i < this.shape[0]; i++) {
      for (var k = 0; k < this.shape[1]; k++) {
        const thisElem = this.get([i, k]);
        const rowOffset = i * other.shape[1];
        for (var j = 0; j < other.shape[1]; j++) {
          data[rowOffset + j] += thisElem * other.get([k, j]);
        }
      }
    }
