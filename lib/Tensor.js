...
class Tensor {
  ...
  reshape(shape) {
    if (generalUtils.prod(shape) !== generalUtils.prod(this.shape)) {
      throw new ShapeError(`Unable to reshape ${this.shape} tensor into shape ${shape}`);
    }
  ...
  //tile, either by adding new dimensions or expanding old ones
  //addDims should rarely be used - instead use broadcastOn
  //multiples is a list of lists of length 2 ([idx, multiple])
  tile(multiples, options) {
    const { addDims, knownSafe } = generalUtils.defaults(options, {
      addDims: false,
      knownSafe: false,
    });

    if (!knownSafe) {
      multiples.forEach((kv) => {
        kv.forEach((num) => {
          if (!Number.isInteger(num) || num < 0) {
            throw new Error(`Invalid multiples object: ${multiples}`);
          }
        });
        const k = kv[0];
        if (k >= this.dims + multiples.length) {
          throw new Error(`Multiples object index too large: ${multiples}`);
        }
      });
    }
    ...
    if (addDims) {
      let thisIdx = 0;
      for (let idx = 0; idx < this.dims + multiples.length; idx++) {
        if (idx in multiplesObj) {
          finalShape.push(multiplesObj[idx]);
        } else {
          finalShape.push(this.shape[thisIdx]);
          thisIdx++;
        }
      }
      const result = Tensor.fromDataAndShape(
        Array(generalUtils.prod(finalShape)),
        finalShape
      );
      const broadcasted = this.broadcastOn(...multiples.map((kv) => kv[0]));
      return result.assign(broadcasted);
    } else {
      for (let idx = 0; idx < this.dims; idx++) {
        const mult = (idx in multiplesObj) ? multiplesObj[idx] : 1;
        finalShape.push(this.shape === undefined ? undefined : this.shape[idx] * mult);
      }

      const newMultiples = [];
      multiples.sort((a, b) => a[0] - b[0]).forEach((kv, idxIncr) => {
        const [ k, v ] = kv;
        newMultiples.push([k + idxIncr, v]);
      });
      const withExtraDims = this.tile(newMultiples, {
        addDims: true,
        knownSafe: true,
      });
      return Tensor.fromDataAndShape(withExtraDims.data, finalShape);
    }
  }
  ...
  reduceSingleAxisToIndex(
    ...
    const { strides: jStrides, mods: jMods } = tensorUtils.getStridesAndMods(
      this.shape
    );
