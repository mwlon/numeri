...
class Tensor {
  ...
  //updateFn: (accumulator, val, idx) => newAccumulator
  ...
  reduceSingleAxis(
    ...
    const { axis, keepScalarAsTensor } = generalUtils.defaults(options, {
    ...
    const resultData = Array(outLength);
    for (let i = 0; i < outLength; i++) {
      resultData[i] = initializeFn();
    }
    ...
    for (let j = 0; j < this.getLength(); j++) {
      ...
      resultData[outI] = updateFn(resultData[outI], this.getByJ(j), axisJ);
    }
  ...
  argmax(options) {
    return this.reduceSingleAxis(
      () => ({max: -Infinity, axisJ: -1}),
      (acc, x, axisJ) => {
        if (x > acc.max) {
          acc.max = x;
          acc.axisJ = axisJ;
